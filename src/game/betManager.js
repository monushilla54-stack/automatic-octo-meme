 ...
 * @param {number} params.betEndTime - epoch ms when betting closes
 * @param {object} params.data       - raw bet message from client
 * @returns {{ success: boolean, bet?: object, reason?: string }}
 */
async function processBet({ playerId, roundId, tableId, betEndTime, data }) {
    ...
    // --- 7. Atomic DB write: lock funds + store bet ---
    let bet;
    try {
        await withTransaction(async (client) => {
            await ledgerService.lockBet(playerId, amount, betId, client);
